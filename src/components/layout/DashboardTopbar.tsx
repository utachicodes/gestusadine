  ...
  return (
    <header className="sticky top-0 z-30 flex items-center gap-3 min-h-[4rem] pt-safe px-4 md:px-6 border-b border-border bg-background/80 backdrop-blur-xl">
      ...
      <button
        ...
        className="md:hidden flex items-center justify-center w-11 h-11 rounded-xl text-muted-foreground hover:bg-secondary hover:text-foreground transition-colors"
      >
      ...
      <button
        type="button"
        onClick={() => { setSearchOpen(true); setTimeout(() => mobileSearchRef.current?.focus(), 100); }}
        aria-label={language === 'fr' ? 'Rechercher' : 'Search'}
        className="md:hidden flex items-center justify-center w-11 h-11 rounded-xl text-muted-foreground hover:bg-secondary hover:text-foreground transition-colors ml-auto"
      >
      ...
      {searchOpen && (
        <div className="md:hidden fixed inset-0 z-[60] bg-background/95 backdrop-blur-md flex flex-col pt-safe">
          <div className="flex items-center gap-3 px-4 py-3 border-b border-border">
            <button
              type="button"
              onClick={() => { setSearchOpen(false); setQuery(''); }}
              className="flex items-center justify-center w-11 h-11 rounded-xl text-muted-foreground hover:bg-secondary"
            >
              <X className="w-5 h-5" />
            </button>
            <div className="relative flex-1">
              ...
              <input
                ...
                value={query}
                onChange={(e) => { setQuery(e.target.value); setOpen(true); }}
                onKeyDown={(e) => { if (e.key === 'Enter' && results.length > 0) { go(results[0][0]); setSearchOpen(false); setQuery(''); } }}
                placeholder={language === 'fr' ? 'Rechercher...' : 'Search...'}
                className="w-full h-12 pl-10 pr-4 rounded-xl bg-secondary border border-transparent text-base text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-ring focus:bg-card"
              />
            </div>
          </div>
              ...
              {results.map(([path, m]) => {
                ...
                return (
                  <button
                    key={path}
                    type="button"
                    onClick={() => { go(path); setSearchOpen(false); setQuery(''); }}
                    className="flex items-center gap-3 w-full px-3 py-3 text-left text-sm text-foreground hover:bg-secondary rounded-xl transition-colors"
                  >
                    <Icon className="w-5 h-5 text-muted-foreground flex-shrink-0" />
                    <div className="min-w-0">
                      <span className="block font-medium truncate">{tr(m.title)}</span>
                      <span className="text-xs text-muted-foreground/70">{tr(m.section)}</span>
                    </div>
                  </button>
                );
              })}
